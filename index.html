<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <!-- Bootstrap CSS -->
    <link
      href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/css/bootstrap.min.css"
      rel="stylesheet"
      integrity="sha384-eOJMYsd53ii+scO/bJGFsiCZc+5NDVN2yr8+0RDqr0Ql0h+rP48ckxlpbzKgwra6"
      crossorigin="anonymous"
    />

    <!-- Bootstrap Icons -->
    <link
      rel="stylesheet"
      href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.4.1/font/bootstrap-icons.css"
    />

    <!-- Custom Style CSS -->
    <link rel="stylesheet" href="./assets/css/style.css" type="text/css" />
    
    <link href="<?php echo base_url('assets/img/brand/Tempt.png') ?>" rel="shortcut icon">
    <title>Temperature Coffee</title>
  </head>
  <body>
    <!-- Popup Start -->
    <!-- <div class="container-fluid" id="popupAds">
      <i class="bi bi-x" id="closePopup"></i>
      <div class="row justify-content-center">
        <div
          class="
            col-md-4 col-lg-3 col-xl-2
            d-flex
            align-items-center
            justify-content-center
          "
        >
          <div class="heading d-flex align-items-center justify-content-center">
            <img src="./assets/img/ornament/surprise.svg" alt="" />
            <h3 class="caption">We've 75% discount for all product</h3>
          </div>
        </div>
        <div class="col-md-4 col-lg-3 col-xl-2 my-2">
          <div id="countdown">
            <ul>
              <li><span id="days"></span>day</li>
              <li><span id="hours"></span>Hour</li>
              <li><span id="minutes"></span>Min</li>
              <li><span id="seconds"></span>Sec</li>
            </ul>
          </div>
        </div>
        <div class="col-md-4 col-lg-3 col-xl-2 text-center my-auto">
          <button type="button" class="btn btn-fzn btn-warning">Get Now</button>
        </div>
      </div>
    </div>
    Popup End -->

    <!-- Header Start -->
    <div class="header">
      <nav class="navbar navbar-expand-lg navbar-light">
        <div class="container">
          <a class="navbar-brand" href="#">
            <img src="./assets/img/brand/Tempt.png" alt="" />
          </a>
          <button
            class="navbar-toggler"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#navbarSupportedContent"
            aria-controls="navbarSupportedContent"
            aria-expanded="false"
            aria-label="Toggle navigation"
          >
            <span class="navbar-toggler-icon"></span>
          </button>
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
              <li class="nav-item">
              <a href="<?= site_url('/') ?>" class="nav-link active">Home</a>
              </li>
              <li class="nav-item">
                <a href =" <?= site_url('Menucontroller') ?>"class="nav-link">Daftar Menu</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#">Tentang</a>
              </li>
              <!-- <li class="nav-item">
                <a class="nav-link" href="#"
                  >Contact Us
                  <svg
                    width="16"
                    height="17"
                    viewBox="0 0 16 17"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                  >
                    <path d="M0 16.62H16V0.62H0V16.62Z" fill="url(#pattern0)" />
                    <defs>
                      <pattern
                        id="pattern0"
                        patternContentUnits="objectBoundingBox"
                        width="1"
                        height="1"
                      >
                        <use xlink:href="#image0" transform="scale(0.015625)" />
                      </pattern>
                      <image
                        id="image0"
                        width="64"
                        height="64"
                        xlink:href="data:image/png;base64,"
                      />
                    </defs>
                  </svg>
                </a>
              </li> -->
            </ul>
            <button class="btn btn-fzn btn-fzn-filled">Login</button>
            <!-- <div class="search-cart d-flex justify-content-between">
              <form class="d-flex">
                <div class="input-group-text">
                  <i class="bi bi-search"></i>
                </div>
                <input
                  type="text"
                  class="form-control"
                  placeholder="Search"
                  aria-label="Text input with checkbox"
                />
              </form>
              <button
                class="cart d-flex justify-content-center align-content-center"
              >
                <span class="item-ordered">2</span>
                <i class="bi bi-cart2"></i>
              </button>
            </div> -->
          </div>
        </div>
      </nav>
    </div>
    <!-- Header End -->
 <!-- Hero Start -->
 <div class="container hero">
      <div class="row align-items-center justify-content-between">
        <div class="col-md-6">
          <!-- <h3 class="subtitle">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M23.5 16H21.9343C21.968 15.8351 22 15.6694 22 15.5C22 14.1933 20.9932 13.1176 19.7138 13.0093C19.2851 12.2578 18.3808 11.8437 17.5361 12.0571C16.8213 11.0473 15.3281 10.7045 14.249 11.3418C13.4084 10.846 12.3273 10.9449 11.5479 11.4931L9.81834 10.1093C9.71484 10.0268 9.58298 9.99116 9.45019 10.0029C9.31833 10.0175 9.19823 10.0839 9.11522 10.1875L7.242 12.5298L6.86034 12.1469C6.76659 12.0532 6.63867 12 6.50583 12C6.37303 12 6.24605 12.0527 6.1523 12.1465L2.2988 16H0.500016C0.223641 16 0 16.2236 0 16.5C0 18.4297 1.57031 20 3.50002 20H20.5C22.4297 20 24 18.4297 24 16.5C24 16.2236 23.7764 16 23.5 16ZM11.7002 12.7695C12.1377 11.9951 13.2295 11.7676 13.9346 12.3384C14.1182 12.4858 14.3799 12.4863 14.5635 12.3379C15.3057 11.7373 16.4746 12.0337 16.8614 12.8804C16.919 13.0049 17.0244 13.1006 17.1533 13.145C17.2822 13.1895 17.4248 13.1797 17.5469 13.1162C18.1016 12.8281 18.7266 13.1152 18.9239 13.6768C18.9942 13.877 19.253 14.001 19.4698 14.001C20.2979 14.0049 21.001 14.6631 21 15.5C21 15.6722 20.9603 15.839 20.9033 16H8.49998C8.49998 15.4488 8.94825 15 9.49997 15C9.77634 15 9.99998 14.7764 9.99998 14.5C9.99998 13.7461 10.5693 13.1099 11.3232 13.0205C11.4824 13.0015 11.6221 12.9082 11.7002 12.7695ZM9.58397 11.2026L10.7613 12.145C9.87802 12.4206 9.20583 13.1458 9.04003 14.0533C8.96742 14.0704 8.89894 14.0974 8.82952 14.1223L7.95281 13.2428L9.58397 11.2026ZM6.50489 13.2075L7.9898 14.6971C7.68684 15.0478 7.5 15.5012 7.5 16H3.71278L6.50489 13.2075ZM20.5 19H3.50002C2.29298 19 1.28222 18.1396 1.04981 17H22.9502C22.7178 18.1396 21.707 19 20.5 19Z"
                fill="#F7F8F7"
              />
              <path
                d="M5.99997 8.49999C5.99997 8.77636 6.22361 9 6.49998 9C6.77636 9 7 8.77636 7 8.49999C7 7.8818 6.83205 7.54636 6.69728 7.27636C6.583 7.04738 6.50003 6.88181 6.50003 6.49997C6.50003 6.11813 6.58305 5.95261 6.69728 5.72358C6.83205 5.45358 7 5.1181 7 4.49995C7 4.22358 6.77636 3.99994 6.49998 3.99994C6.22361 3.99999 5.99997 4.22363 5.99997 4.5C5.99997 4.88185 5.91695 5.04736 5.80272 5.27639C5.66795 5.54639 5.5 5.88188 5.5 6.50002C5.5 7.11816 5.66795 7.45364 5.80272 7.72364C5.91695 7.95263 5.99997 8.11819 5.99997 8.49999Z"
                fill="#F7F8F7"
              />
              <path
                d="M12 8.49999C12 8.77636 12.2236 9 12.5 9C12.7764 9 13 8.77636 13 8.49999C13 7.8818 12.832 7.54636 12.6973 7.27636C12.583 7.04738 12.5 6.88181 12.5 6.49997C12.5 6.11813 12.583 5.95261 12.6973 5.72358C12.832 5.45358 13 5.1181 13 4.49995C13 4.22358 12.7764 3.99994 12.5 3.99994C12.2236 3.99999 12 4.22363 12 4.5C12 4.88185 11.917 5.04736 11.8027 5.27639C11.668 5.54639 11.5 5.88188 11.5 6.50002C11.5 7.11816 11.668 7.45364 11.8027 7.72364C11.917 7.95263 12 8.11819 12 8.49999Z"
                fill="#F7F8F7"
              />
              <path
                d="M18 8.49999C18 8.77636 18.2236 9 18.5 9C18.7764 9 19 8.77636 19 8.49999C19 7.8818 18.832 7.54636 18.6973 7.27636C18.583 7.04738 18.5 6.88181 18.5 6.49997C18.5 6.11813 18.583 5.95261 18.6973 5.72358C18.832 5.45358 19 5.1181 19 4.49995C19 4.22358 18.7764 3.99994 18.5 3.99994C18.2236 3.99994 18 4.22363 18 4.5C18 4.88185 17.917 5.04736 17.8027 5.27639C17.668 5.54639 17.5 5.88188 17.5 6.50002C17.5 7.11816 17.668 7.45364 17.8027 7.72364C17.917 7.95263 18 8.11819 18 8.49999Z"
                fill="#F7F8F7"
              />
            </svg>
            A magic plate for everyone
          </h3> -->
          <h1>
            Temperature <br />
            <span class="lineword">Coffee</span>
            Bandung 
          </h1>
          <h3 class="caption">
            Now, hungry is not problem anymore. We provide delicious food for
            you in everyday
          </h3>
          <button class="btn btn-fzn btn-fzn-filled">Kunjungi</button>
          <button class="btn btn-fzn">
            <i class="bi bi-play-circle-fill"></i> Cara Memesan
          </button>
          <div class="row counter">
            <div class="col-3">
              <h5 data-target="4012" class="count">0</h5>
              <h6 class="label">Pelanggan</h6>
            </div>
            <div class="col-3">
              <h5 data-target="4002" class="count">0</h5>
              <h6 class="label">Rating</h6>
            </div>
            <div class="col-3">
              <h5 data-target="300" class="count">0</h5>
              <h6 class="label">Pengguna</h6>
            </div>
          </div>
        </div>
        <div class="col-md-6 image">
          <img src="./assets/img/brand/barista.png" alt="barista" />
        </div>
      </div>
    </div>
    <!-- Hero End -->

    <!-- Services Start -->
    <!-- <div class="container-fluid not-header services">
      <div class="row text-center mb-3">
        <h3 class="subtitle">we guarantee</h3>
        <h2>Why <span class="lineword">choose</span> us?</h2>
      </div>
      <div class="row text-center justify-content-center">
        <div class="col-md-3 col-xl-2 icon-box">
          <img src="./assets/img/icon/quality.svg" alt="" />
          <h5 class="mt-4">High Quality</h5>
        </div>
        <div class="col-md-3 col-xl-2 icon-box">
          <img src="./assets/img/icon/no-preservative.svg" alt="" />
          <h5 class="mt-4">Fresh</h5>
        </div>
        <div class="col-md-3 col-xl-2 icon-box">
          <img src="./assets/img/icon/professional.svg" alt="" />
          <h5 class="mt-4">Professional</h5>
        </div>
      </div>
    </div> -->
    <!-- Services End -->

    <!-- Feature Start -->
    <!-- <div class="container not-hero not-header feature">
      <div class="row text-center mb-3">
        <h3 class="subtitle">Our Features</h3>
        <h2><span class="lineword">New</span> Feature Released</h2>
      </div>
      <div class="row justify-content-center align-items-end">
        <div class="col-sm-6 col-md-5 col-lg-4 col-xl-3 text left mx-3">
          <div class="row detail">
            <h5 class="title">Food Consultation</h5>
            <p class="desc">
              We help you to find the best way to cook well. We are available
              whenever you want
            </p>
          </div>
          <div class="row detail">
            <h5 class="title">Chas On Delivery</h5>
            <p class="desc">
              Get your food, and pay it when you receive it. Don't wait to
              starve
            </p>
          </div>
          <div class="row detail">
            <h5 class="title">Help The Hungry</h5>
            <p class="desc">
              Have a sense of humanity, be a good people help others in need
            </p>
          </div>
        </div>
        <div
          class="
            col-md-auto
            image
            d-flex
            align-items-start
            order-md-1 order-xl-0
            d-flex
            justify-content-center
          "
        >
          <img src="./assets/img/app/app-chat.png" alt="" />
        </div>
        <div class="col-sm-6 col-md-5 col-lg-4 col-xl-3 text right mx-3">
          <div class="row detail">
            <h5 class="title">Create Own Menu</h5>
            <p class="desc">
              Planning to fatten or make yourself thin? plan it now and don't
              delay anymore
            </p>
          </div>
          <div class="row detail">
            <h5 class="title">Eat Today, Pay Later</h5>
            <p class="desc">
              Have no money but want to eat something? just eat and pay later
            </p>
          </div>
          <div class="row detail">
            <h5 class="title">Call The Chef</h5>
            <p class="desc">
              Do you want to be guided to learn cooking? we have expert chef
            </p>
          </div>
        </div>
      </div>
    </div> -->
    <!-- Feature End -->

    <!-- Product Start -->
    <div class="container not-hero not-header feature product">
      <div class="row mb-4">
        <div class="col heading d-flex">
          <h2>Reko<span class="lineword">mendasi</span></h2>
          <a class="btn btn-fzn">
            View All <i class="bi bi-arrow-right"></i>
          </a>
        </div>
      </div>
      <div class="row">
        <div class="col d-flex justify-content-between card-wrapper">
          <div class="card">
            <div class="product-img">
              <img
                src="./assets/img/product/kopisusupandan.png"
                class="card-img-top"
              />
            </div>
            <div class="header">
              <p class="availability">In Stock</p>
              <p class="like d-flex justify-content-center align-items-center">
                <i class="bi bi-heart-fill"></i>
              </p>
            </div>
            <div class="card-body">
              <h5 class="card-title">Es Kopi Susu Pandan</h5>
              <div
                class="
                  card-meta
                  d-flex
                  align-items-center
                  justify-content-start
                "
              >
                <p class="category">Minuman</p>
                <p class="star mx-3"><i class="bi bi-star-fill"></i> 4.9</p>
              </div>
              <div
                class="footer d-flex align-items-center justify-content-between"
              >
                <h3 class="price">Rp. 18.000</h3>
                <a href="#" class="btn btn-cart"><i class="bi bi-cart2"></i></a>
              </div>
            </div>
          </div>
          <div class="card">
            <div class="product-img">
              <img
                src="./assets/img/product/icedcaramel2.jpg"
                class="card-img-top"
              />
            </div>
            <div class="header">
              <p class="availability">In Stock</p>
              <p class="like d-flex justify-content-center align-items-center">
                <i class="bi bi-heart-fill"></i>
              </p>
            </div>
            <div class="card-body">
              <h5 class="card-title">Ice Caramel Machiato</h5>
              <div
                class="
                  card-meta
                  d-flex
                  align-items-center
                  justify-content-start
                "
              >
                <p class="category">Minuman</p>
                <p class="star mx-3"><i class="bi bi-star-half"></i> 3</p>
              </div>
              <div
                class="footer d-flex align-items-center justify-content-between"
              >
                <h3 class="price">Rp. 20.000</h3>
                <a href="#" class="btn btn-cart"><i class="bi bi-cart2"></i></a>
              </div>
            </div>
          </div>
          <div class="card">
            <div class="product-img">
              <img
                src="./assets/img/product/kopisusu.jpg"
                class="card-img-top"
              />
            </div>
            <div class="header">
              <p class="availability">In Stock</p>
              <p class="like d-flex justify-content-center align-items-center">
                <i class="bi bi-heart-fill"></i>
              </p>
            </div>
            <div class="card-body">
              <h5 class="card-title">Es Kopi Susu</h5>
              <div
                class="
                  card-meta
                  d-flex
                  align-items-center
                  justify-content-start
                "
              >
                <p class="category">Minuman</p>
                <p class="star mx-3"><i class="bi bi-star-fill"></i> 5.0</p>
              </div>
              <div
                class="footer d-flex align-items-center justify-content-between"
              >
                <h3 class="price">Rp. 21.000</h3>
                <a href="#" class="btn btn-cart"><i class="bi bi-cart2"></i></a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- Product End -->

    <!-- Discount Start -->
    <!-- <div
      class="
        container-fluid
        not-hero not-header
        discount
        d-flex
        justify-content-center
      "
    >
      <div class="container">
        <div class="row">
          <div class="col-md-5 heading">
            <h1 class="amount">Discount 75%</h1>
            <h3 class="coupon">COUPON: <span>SPRNGDAY</span></h3>
            <p class="date">Valid until 20 days later</p>
            <button class="btn btn-fzn btn-fzn-filled my-3">
              Use The Coupon
            </button>
          </div>
          <div class="col-md-7 image">
            <img src="./assets/img/chef/chef-greting.png" alt="" />
            <img src="./assets/img/chef/chef-lookup.png" alt="" />
          </div>
        </div>
      </div>
    </div> -->
    <!-- Discount End -->

    <!-- Team Start -->
    <!-- <div class="container not-hero not-header team">
      <div class="row mb-4 text-center">
        <h3 class="subtitle">A Solid Team</h3>
        <h2><span class="lineword">Professional</span> Barista Our Team</h2>
      </div>
      <div class="row">
        <div class="col-6 col-xl-3 team-box">
          <img src="./assets/img/chef/team1.jpg" alt="" class="pict-team" />
          <div class="team-info">
            <h5 class="team-name">Fernando Abra</h5>
            <p class="team-job">CEO</p>
          </div>
        </div>
        <div class="col-6 col-xl-3 team-box">
          <img src="./assets/img/chef/team2.jpg" alt="" class="pict-team" />
          <div class="team-info">
            <h5 class="team-name">Cecep Sumacep</h5>
            <p class="team-job">Barista</p>
          </div>
        </div>
        <div class="col-6 col-xl-3 team-box">
          <img src="./assets/img/chef/team3.jpg" alt="" class="pict-team" />
          <div class="team-info">
            <h5 class="team-name">Redona Leslie</h5>
            <p class="team-job">Barista</p>
          </div>
        </div>
        <div class="col-6 col-xl-3 team-box">
          <img src="./assets/img/chef/team4.jpg" alt="" class="pict-team" />
          <div class="team-info">
            <h5 class="team-name">David Cooper</h5>
            <p class="team-job">Barista</p>
          </div>
        </div>
      </div> -->
     <!--  <div class="row">
        <div class="col-6 col-xl-3 team-box">
          <img src="./assets/img/chef/team5.jpg" alt="" class="pict-team" />
          <div class="team-info">
            <h5 class="team-name">Ferdinan Maarola</h5>
            <p class="team-job">Barista</p>
          </div>
        </div>
        <div class="col-6 col-xl-3 team-box">
          <img src="./assets/img/chef/team6.jpg" alt="" class="pict-team" />
          <div class="team-info">
            <h5 class="team-name">Titi Maryati</h5>
            <p class="team-job">Barista</p>
          </div>
        </div>
        <div class="col-6 col-xl-3 team-box">
          <img src="./assets/img/chef/team7.jpg" alt="" class="pict-team" />
          <div class="team-info">
            <h5 class="team-name">Brandon Ant</h5>
            <p class="team-job">Barista</p>
          </div>
        </div> -->
        <!-- <div class="col-6 col-xl-3 team-box">
          <div class="team-join">
            <h5>Do you have cooking skill?</h5>
            <a> Join Us Now <i class="bi bi-arrow-right"></i> </a>
          </div>
        </div>
      </div> -->
    <!-- </div> -->
    <!-- Team End -->

    <!-- App Start -->
    <!-- <div class="container-fluid not-header not-hero app">
      <div class="container">
        <div class="row">
          <div class="col-12 col-lg-7 align-items-center">
            <h2>Download Our App</h2>
            <div class="row">
              <a
                href="#"
                class="
                  btn
                  col-6 col-lg-5
                  store
                  d-flex
                  align-items-center
                  justify-content-around
                "
              >
                <img src="./assets/img/brand/appstore.png" alt="" />
                <h5>App Store</h5>
              </a>
              <a
                href="#"
                class="
                  btn
                  col-6 col-lg-5
                  store
                  d-flex
                  align-items-center
                  justify-content-around
                "
              >
                <img src="./assets/img/brand/playstore.png" alt="" />
                <h5>Play Store</h5>
              </a>
            </div>
          </div>
          <div class="col-12 col-lg-5 appmockup">
            <img src="./assets/img/app/app-home.png" alt="" />
          </div>
        </div>
      </div>
    </div> -->
    <!-- App End -->

    <!-- Footer Start -->
    <div class="container-fluid not-header not-hero footer">
      <div class="container">
        <div class="row">
          <div class="col-xl-4 brand">
            <img src="./assets/img/brand/Tempt.png" alt="" />
            <!-- <h3 class="slogan">“a delicious plate”</h3> -->
            <div class="contact">
              <p class="address">
                <i class="bi bi-map"></i> Jl. Dipati Ukur No.272, Lebakgede, Kecamatan Coblong,
                 Kota Bandung, Jawa Barat 40132
              </p>
              <p class="phone">
                <i class="bi bi-telephone"></i> +62 819-3127-4079
              </p>
              <p class="email">
                <i class="bi bi-envelope"></i> ctemperature@gmail.com
              </p>
            </div>
          </div>
          <div class="col-xl-6 links">
            <div class="row">
              <div class="col-md-3 menu-footer">
                <h6 class="title">Kedai Coffe</h6>
                <ul class="footer-nav">
                  <li class="link-item"><a href="#">Tentang</a></li>
                  <!-- <li class="link-item"><a href="#">Our Barista</a></li> -->
                </ul>
              </div>
              <div class="col-md-3 menu-footer">
                <h6 class="title">Kategori</h6>
                <ul class="footer-nav">
                  <li class="link-item"><a href="#">Minuman</a></li>
                </ul>
              </div>
              <div class="col-md-3 menu-footer">
                <h6 class="title">Ulasan</h6>
                <ul class="footer-nav">
                  <li class="link-item"><a href="#">Ulasan & Rating</a></li>
                </ul>
              </div>
              <div class="col-md-3 menu-footer">
                <h6 class="title">Ikuti Kami</h6>
                <ul class="footer-nav">
                  <li class="link-item">
                    <a href="https://www.instagram.com/temperature.coffee/">Instagram</a>
                    <!-- <a href="https://instagram.com/fazanash">Instagram</a> -->
                  </li>
                  <!-- <li class="link-item">
                    <a href="https://facebook.com/fazanash">Facebook</a>
                  </li> -->
                  <li class="link-item"><a href="#">Youtube</a></li>
                </ul>
              </div>
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col divider"></div>
        </div>
        <div class="row">
          <div class="col">
            <div class="meta copyright">
              <p>Copyright &copy; 2021 CTemperature</p>
            </div>
            <div class="meta menu-footer-bottom">
              <ul class="legal-menu">
                <li class="link-item"><a href="#">Privacy Policy</a></li>
                <li class="link-item"><a href="#">Term of Use</a></li>
                <li class="link-item"><a href="#">Legal Info</a></li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- Footer End -->

    <!-- Download -->
    <!-- <div class="container-fluid download">
      <a href="https://github.com/fazanash"
        ><i class="bi bi-cloud-download"></i> Download <span>Free</span></a
      >
    </div> -->

    <!-- JAVASCRIPT -->
    <!-- Optional JavaScript; choose one of the two! -->
    <!-- Option 1: Bootstrap Bundle with Popper -->
    <script
      src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta3/dist/js/bootstrap.bundle.min.js"
      integrity="sha384-JEW9xMcG8R+pH31jmWH6WWP0WintQrMb4s7ZOdauHnUtxwoG2vI5DkLtS3qm9Ekf"
      crossorigin="anonymous"
    ></script>
    <script src="./assets/js/countdown.js"></script>
    <script src="./assets/js/campaign.js"></script>
    <script src="./assets/js/counter.js"></script>
  </body>
</html>
